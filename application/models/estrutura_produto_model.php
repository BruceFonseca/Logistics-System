...
class Estrutura_produto_model extends CI_Model{
   ...
   //  		$sql = "DELETE from estrutura";
            ...
            $csv_array = $this->csvimport->get_array($file_path, array('produto', 'componente', 'descricao', 'quantidade', 'UM'));

			foreach ($csv_array as $row)
			{
				$cd_produto= substr($row['produto'], 0, 9);
				$cd_componente= $row['componente']; 
				$dsc_componente= $row['descricao'];
				$quantidade= floatval($row['quantidade']);
				$um = $row['UM'];

				//se o componente não for NULL, então adiciona na query
			        $query = "INSERT INTO estrutura (cd_produto, cd_componente, dsc_componente, quantidade, um ) VALUES 
							('". $cd_produto. "'". "," . "'". $cd_componente. "'". "," . "'". $dsc_componente. "'". "," . $quantidade .  "," .  "'" . $um .  "'" .  ")";
					
					$this->db->query($query);// insere no banco de dados
			}
	...
	public function insert_csv($insert_data=NULL){
		
		if($insert_data !=NULL){
			$this->db->insert('estrutura', $insert_data);
		}
